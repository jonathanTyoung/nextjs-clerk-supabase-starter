...
export async function POST(req: NextRequest) {
  const { staffId, notes } = await req.json()

  // Fetch staff + employee + event data
  ...
  const { error: sendError } = await resend.emails.send({
    ...
    to: employee.email,
    subject: `Availability Request — ${event.title} on ${eventDate}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto; color: #1a1a1a;">
        <p>Hi ${employee.name},</p>
        <p>We'd like to check your availability for an upcoming event. Please review the details below and reply to this email to let us know if you're available.</p>
        ...
        ${notes ? `<p style="background: #fff8e1; border-left: 3px solid #f0b429; padding: 12px 16px; border-radius: 4px;"><strong>Additional notes:</strong><br/>${notes}</p>` : ''}

        <p>Please reply to this email to confirm your availability. If you have any questions, don't hesitate to reach out.</p>
